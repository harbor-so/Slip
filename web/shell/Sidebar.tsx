		...
		<nav className="flex w-60 shrink-0 flex-col border-r border-border bg-card">
			...
			<div className="mt-6 flex min-h-0 flex-1 flex-col px-2">
				...
				<div className="min-h-0 flex-1 overflow-y-auto">
					{connectors.length === 0 ? (
						<Link href="/connectors" className="block px-2.5 text-xs text-primary hover:underline">
							...
							{connectors.map((account) => (
								<li key={account.type}>
									...
									>
										...
										<span className="nums ml-auto truncate text-[10px] text-muted-foreground/70">
											{account.status === "error"
												? "error"
			...
			<div className="border-t border-border p-3">
				<div className="flex items-center gap-2">
					...
					<div className="min-w-0">
						<p className="truncate text-xs text-foreground">{orgName ?? "no org"}</p>
