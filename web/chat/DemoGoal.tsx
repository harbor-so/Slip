...
import { ArrowRight, CornerDownLeft, FileAudio, Link2, Paperclip, Users } from "lucide-react";
...
/** The sample ask that Tab auto-types, the way the reference demo does it. */
const SAMPLE_PROMPT =
	"We keep losing deals because prospects can't see a board without an account. Add public, view-only share links to project boards — no login, read-only, and revocable per board.";
...
export function DemoGoal() {
	...
	// Tab auto-types the sample ask, character by character, so a viewer can see
	// what prompting Harbor looks like without typing it themselves.
	function autofill() {
		if (timerRef.current) return;
		let cursor = 0;
		timerRef.current = setInterval(() => {
			cursor = Math.min(cursor + 6, SAMPLE_PROMPT.length);
			setValue(SAMPLE_PROMPT.slice(0, cursor));
			if (cursor >= SAMPLE_PROMPT.length && timerRef.current) {
				...
				<div className="w-full max-w-2xl">
					...
					<div className="rounded-xl border border-border bg-background">
						<textarea
							...
							placeholder="Ask or build anything…  (press Tab for a sample prompt)"
							value={value}
						/>
