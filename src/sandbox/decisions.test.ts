...
import { setting, type RepoOverrides } from "../config.js";
...
describe("module boundary", () => {
	...
	it("imports only from contracts and config", () => {
		expect(new Set(specifiers)).toEqual(new Set(["../config.js", "../contracts/index.js"]));
	});

	it("imports nothing from the manager, the database or any driver", () => {
		for (const specifier of specifiers) {
			expect(specifier).not.toMatch(/manager|\/db\/|drizzle|postgres|node:/);
		}
	});
