 ...
 *
 * This is the idea the whole chat model turns on: a human and an agent are the
 * same kind of thing — a keypair — so a channel never has to distinguish them,
 * and agent↔agent, agent↔human and human↔human are one primitive rather than
 * three. `pubkey` is the Ed25519 public key in hex and is the identity used in
 * every signed event; `kind` is display sugar, not a permission.
 *
 ...
 * can leak in full and forge nothing.
 *
 * `userId` links a human principal back to its dashboard login as provenance —
 * it says which GitHub user registered this key. An agent's principal has no
 * user; it was registered under an org API key, which is the whole of its
 * authority.
 */
export const principals = pgTable(
	...
	(table) => [
		// Scoped to the org: the same person joining two tenants registers a key in
		// each, and a pubkey is only ever resolved together with the org the
		// connection already proved. Two orgs holding the same pubkey is not a
 ...
 * nothing.
 *
 * `kind` shapes how access is granted, never who may be in the room:
 *   - `group`  — possession of `key` is the credential; opening the link joins.
 *   - `direct` — a fixed roster (a DM); membership is the gate, the key is not
 *     handed around.
 *   - `task`   — bound to a Harbor task, otherwise a group.
 *
 * `nextSeq` is the same row-locked counter sessions uses, for the same reason:
 * `max(seq)+1` races two concurrent posts into the same number and drops one.
 */
 ...
 * returned, so a private channel cannot leak to a non-member — the check is
 * structural, not a `WHERE` someone can forget. Joining is idempotent (opening a
 * link twice is the common case).
 *
 * `lastSeenSeq` is a per-member read cursor. It is what lets an agent pull
 * everything said since it last looked in a single batch — buzz's "batch queued
 * mentions into one prompt" — rather than one message per turn, and it doubles as
 * the unread count for humans.
 */
 ...
 * `ON CONFLICT DO NOTHING` absorbs the duplicate. buzz gets this from a
 * `(community, id)` write-conflict scope; the id is a 256-bit hash of a body that
 * already includes the channel, so a plain primary key is enough here.
 *
 * `sig` is nullable, and the split is deliberate. Content events — `message`,
 * `reaction` — are end-to-end signed by their author and carry a `sig`.
 * Membership and system events — `join`, `leave`, `system`, `channel_create` —
 * are authored by the server in response to an already-authenticated action
 * (opening a link, creating a room) and have no signature to give. Ephemeral
 * kinds (`typing`, `presence`) are never written here at all.
 *
 * `authoredAt` is the author's own clock from the signed body; `ingestedAt` is
 * when the server accepted it. Ordering never uses either — it uses `seq`, which
 * the server assigns, so a lying clock cannot reorder a conversation.
 */
